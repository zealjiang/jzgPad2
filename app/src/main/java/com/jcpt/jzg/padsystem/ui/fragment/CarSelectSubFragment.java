 ...
 * @author zealjiang
 */
public class CarSelectSubFragment extends BaseFragment implements CarTypeListView.OnListDataLoadOverListener,CarDiffSelectView.OnDiffSelectedItemListener{
    ...
    CarTypeListView carTypeListView;
    ...
    /**
     * isModified 用来标示数据是否有修改，如果有修改为true，没修改为false
     */
    private boolean isModified = false;//数据是否修改，修改过为true，反之为false
    ...
    protected View initViews(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_car_select_sub, container, false);
        ButterKnife.bind(this, view);
        EventBus.getDefault().register(this);
        detectMainActivity = (DetectMainActivity)this.getActivity();
        carTypeListView.setDetectMainActivity(detectMainActivity);
        carTypeListView.setOnListDataLoadOverListener(this);
        carDiffSelectView.setOnDiffSelectedItemListener(this);
        carDiffSelectView.setDetectMainActivity(detectMainActivity);

        initDataAfterVisible();

        if(localCarConfigModel!=null){
            tvTitle.setText(localCarConfigModel.getStyleFullName());
        }
        TaskDetailModel taskDetailModel = detectMainActivity.getTaskDetailModel();
    ...
    private void initDataAfterVisible() {
        ...
        //判断这个任务是否存在检测详情数据
        TaskDetailModel taskDetailModel = ((DetectMainActivity)this.getActivity()).getTaskDetailModel();
        ...
        if(submitModel.getMakeID()>0&&submitModel.getModelID()>0&&submitModel.getStyleID()>0){
            ...
            ((DetectMainActivity)CarSelectSubFragment.this.getActivity()).remove(2);
            ((DetectMainActivity)CarSelectSubFragment.this.getActivity()).remove(3);
        }else{
            //设置车况检测和车辆照片tabView不可点击
            ((DetectMainActivity)CarSelectSubFragment.this.getActivity()).add(2);
            ((DetectMainActivity)CarSelectSubFragment.this.getActivity()).add(3);
        }
        ...
        if(submitModel.getVin()!=null&&submitModel.getVin().length()>0&&submitModel.getProductionTime()!=null&&submitModel.getProductionTime().length()>0){
            if(submitModel.getMakeID()>0&&submitModel.getModelID()>0){
                ((DetectMainActivity)CarSelectSubFragment.this.getActivity()).remove(1);
            }
        }
    }
    ...
    @OnClick({R.id.crb_modify,R.id.crbShowConfig,R.id.crbSelectCar})
