...
public class BMWDefectSelectRecyclerViewAdapter extends RecyclerView.Adapter<BMWDefectSelectRecyclerViewAdapter.ViewHolder> {
    ...
    public BMWDefectSelectRecyclerViewAdapter(Activity activity, List<DefectType> defectTypeList, String taskId, String tvName) {
        ...
        SubmitModel submitModel = BMWDetectMainActivity.detectMainActivity.getSubmitModel();
...
//            initListener(tagFlowLayout,llPhotos,sdv_defect1,sdv_defect2,sdv_defect3);
            ...
            public void recapturePhoto(Intent data) {
                if (null != data) {
                    ...
                    if (recapture) {
                        ...
                        if(picDefectHttpUrlList!=null&&picDefectHttpUrlList.size()>position){
                            String path = picDefectHttpUrlList.get(position).getPicPath();
                            if(!StringUtils.isEmpty(path)){
                                if(path.startsWith("http")){
                                    picDefectHttpUrlList.get(position).setPicPath("");
                                }
                            }
                        }
                    } else {
    ...
    private void skipToPictureZoomActivity(String picPath, String photoName, String photoId) {
        ...
        try {
            ((BMWDetectMainActivity) activity ).startActivityForResult(intent, detectionModel
                    .DEFECT_ITEM_RECAPTURE);
        }catch (Exception e){
            ((BMWSearchCheckActivity) activity).startActivityForResult(intent, detectionModel.DEFECT_ITEM_RECAPTURE);
        }
