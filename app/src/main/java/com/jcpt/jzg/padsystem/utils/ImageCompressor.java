...
public class ImageCompressor {
    ...
    private Bitmap compress(String imagePath, int width, int height) {
        ...
        //计算inSampleSize
        int maxL = width>height ? width:height;
        if(maxL<=2048){//相机设置的输入图片的最佳分辨率是2048*1536，见CameraActivity
            options.inSampleSize = 1;
        }else {
            options.inJustDecodeBounds = true;
            BitmapFactory.decodeFile(imagePath, options);
            ...
            if (max <= 2048 && options.inSampleSize > 2) {
    ...
    private File compress(String largeImagePath, String thumbFilePath, int width, int height, int angle, long size) {
        ...
        return saveImage(largeImagePath,thumbFilePath, thbBitmap, size+130);//加100是为了提高照片质量
    }
     ...
     */
    private File saveImage(String largeImagePath, String filePath, Bitmap bitmap, long size) {
        ...
        File largeFile = new File(largeImagePath);
        if(new File(filePath).length()>=largeFile.length()){//如果压缩后的图片比原图还大，则直接将原图复制并返回
            new File(filePath).delete();
            boolean isSuc = FileUtils.copyFile(largeFile.getPath(),filePath);
            if(isSuc){
    ...
    private File compressImage(@NonNull File file) {
        ...
        if(file.length()/1024<=MaxSize){
            ...
            boolean isSuc = FileUtils.copyFile(file.getPath(),thumb);
            if(isSuc){
        ...
        if(new File(thumb).length()>=file.length()){//如果压缩后的图片比原图还大，则直接将原图复制并返回
