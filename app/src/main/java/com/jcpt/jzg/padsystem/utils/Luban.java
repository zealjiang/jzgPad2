...
public class Luban implements Handler.Callback {
  ...
  public static final int DEFALU_COMPRESS = 0;//默认压缩
  public static final int HIGH_QUALITY_COMPRESS = 1;//高质量压缩
  private static int compressLevel = 0;//压缩级别
  ...
  @UiThread private void launch(final Context context) {
    if ((file == null || !file.exists())&& onCompressListener != null) {
    ...
    new Thread(new Runnable() {
      @Override public void run() {
        try {
          ...
          File result;
          if(compressLevel==DEFALU_COMPRESS){
            result = new Engine(file, getImageCacheFile(context)).compress();
          }else{
            result = new Engine(file, getImageCacheFile(context)).highQualityCompress();
          }
  ...
  public static class Builder {
    ...
    public Builder setCompressLevel(int level){
      switch (level){
        case DEFALU_COMPRESS:
          compressLevel = DEFALU_COMPRESS;
          break;
        case HIGH_QUALITY_COMPRESS:
          compressLevel = HIGH_QUALITY_COMPRESS;
          break;
        default:
          compressLevel = DEFALU_COMPRESS;
          break;
