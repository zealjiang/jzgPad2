...
import com.jcpt.jzg.padsystem.ui.fragment.CarConfigSubFragment;
import com.jcpt.jzg.padsystem.ui.fragment.CarSelectSubFragment;
 ...
 */
public class BMWCarSelectFragment extends BaseFragment{
    ...
    BMWCarSelectSubFragment carSelectSubFragment;
    BMWCarConfigSubFragment carConfigSubFragment;
    ...
    public void init(){
        carSelectSubFragment = new BMWCarSelectSubFragment();
        carConfigSubFragment = new BMWCarConfigSubFragment();
        ...
        //获取方案 判断是否有配置项
        DetectionWrapper detectionWrapper = ((BMWDetectMainActivity)this.getActivity()).getWrapper();
    ...
    public boolean nextToDetect(int index){
        ...
        if(isSaveSuc){
            //统计车型选择结束时间
            ((BMWDetectMainActivity) this.getActivity()).recordCarSelectET();

            //设置车况检测和车辆照片tabView可点击
            ((BMWDetectMainActivity)BMWCarSelectFragment.this.getActivity()).remove(2);
            ((BMWDetectMainActivity)BMWCarSelectFragment.this.getActivity()).remove(3);

            if(index==2||index==3) {
                //统计车况检测开始时间
                ((BMWDetectMainActivity) this.getActivity()).recordDetectionST();

                //跳转到车况检测
                ((BMWDetectMainActivity) this.getActivity()).skipToFragment(index);
            }
            return true;
        }else{
            //设置车况检测和车辆照片tabView不可点击
            ((BMWDetectMainActivity)BMWCarSelectFragment.this.getActivity()).add(2);
            ((BMWDetectMainActivity)BMWCarSelectFragment.this.getActivity()).add(3);
            return false;
        }
    }
