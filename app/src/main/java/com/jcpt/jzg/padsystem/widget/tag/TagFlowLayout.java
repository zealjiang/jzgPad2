...
public class TagFlowLayout extends FlowLayout implements TagAdapter.OnDataChangedListener {
    ...
    public interface OnTagClickListener {
        boolean onTagClick(TagView view, int position, FlowLayout parent);
    }
    ...
    /**
     * 模拟点击某个tagView事件，即mOnSelectListener事件
     * zj
     */
    public void afterPerformClick(Set<Integer> set){
        if(set == null){
            return;
        }

        if (mOnSelectListener != null) {
            mOnSelectListener.onSelected(set);
        }
    ...
    private void removeErrorCheckedPos(){
        ...
        for (int value : mSelectedView) {
            if(value>mTagAdapter.getCount()-1){
        ...
        for (int i = 0; i < temp.size(); i++) {
            mSelectedView.remove(temp.get(i));
        }
    }
