package com.jcpt.jzg.padsystem.bmw.model;
...
import com.jcpt.jzg.padsystem.adapter.BMWDefectSelectRecyclerViewAdapter;
 ...
 * <p>
 * 车况检测-- 初始化大对象 （BMWDetectMainActivity --> localDetectionData）
 * 所有的操作将实时反馈写入到初始化大对象里，往后回显和需要取数据均从大对象里取出
 *
 * @Email: libo@jingzhengu.com
 * @Description: 车况检测逻辑 Model
 */
public class BMWDetectionModel {
    ...
    public BMWDetectionModel(Activity activity) {
        this.activity = activity;

        if (BMWDetectMainActivity.detectMainActivity != null && BMWDetectMainActivity.detectMainActivity.getSubmitModel() != null)
            defectValues = (ArrayList<String>) BMWDetectMainActivity.detectMainActivity.getSubmitModel().getDefectValue();
    ...
    public void showDefectPopWindow(List<CheckItem> checkItemList, int curClickPosition) {
        {
            ...
            BMWDefectSelectRecyclerViewAdapter adapter = new BMWDefectSelectRecyclerViewAdapter
                    (activity, defectTypeList, taskId, checkItemList.get(curClickPosition).getCheckName());
...
////        ((BMWDetectMainActivity) context).startActivityForResult(intent, requestCode);
//        try {
//            ((BMWDetectMainActivity) activity).startActivityForResult(intent, requestCode);
//        }catch (Exception e){
...
//        }
        ...
        try {
            ...
            ((BMWSearchCheckActivity) activity).startActivityForResult(intent, requestCode);
        }
...
//        ((BMWDetectMainActivity) context).startActivityForResult(intent, PHOTO_REQUEST);
        ...
        try {
            ((BMWDetectMainActivity) activity).startActivityForResult(intent, PHOTO_REQUEST);
        }catch (Exception e){
            ((BMWSearchCheckActivity) activity).startActivityForResult(intent, PHOTO_REQUEST);
        }
    }

    //保存数据到数据库
    public void saveUseTaskToDB() {
        ArrayList<String> submitDefectValues = (ArrayList<String>) BMWDetectMainActivity.detectMainActivity.getSubmitModel().getDefectValue();
        BMWDetectMainActivity.detectMainActivity.getLocalDetectionData().setDefectValueList(submitDefectValues);
        String json = new Gson().toJson(BMWDetectMainActivity.detectMainActivity.getLocalDetectionData());
     ...
     * 具体做法：以详情数据为参照，遍历数据详情与大对象（BMWDetectMainActivity.LocalDetectionData）对比，有匹配的就把详情里的值设置到 大对象里
     *
    ...
    public void equalsDailDataToPadSysAppWrapper(TaskDetailModel taskDetailModel) {
        ...
        List<CheckPositionItem> checkPositionList = BMWDetectMainActivity.detectMainActivity.getLocalDetectionData().getCheckPositionList();
...
//        List<CheckPositionItem> checkPositionItems = ((BMWDetectMainActivity) context).getLocalDetectionData().getCheckPositionList();
        List<CheckPositionItem> checkPositionItems = BMWDetectMainActivity.detectMainActivity.getLocalDetectionData().getCheckPositionList();

        //遍历所有检测方位 基本 照片
//        List<PictureItem> pictureItems = ((BMWDetectMainActivity) context).getLocalDetectionData().getPictureList();
        List<PictureItem> pictureItems = BMWDetectMainActivity.detectMainActivity.getLocalDetectionData().getPictureList();
