package com.jcpt.jzg.padsystem.db;
...
/**
 * Created by voiceofnet on 2016/9/18.
 */
public class DBManager implements IDB{
        ...
        if(instance==null){
            synchronized (DBManager.class){
                if(instance==null){
                    ...
                    helper = new PadDBHelper(PadSysApp.getAppContext());
                }
            }
        }
    ...
    public boolean isExist(String dataType,String taskId,int userId){
        ...
        String[] selectionArgs = {taskId+"",String.valueOf(userId),dataType+""};//+""是为了防止出现null
     ...
     * @return
     */
    public boolean deletePlan(String planId,String dataType){
        long id = -1;
        String selection = COL_TASK_ID + "=? AND "+ COL_DATA_TYPE + "=?";
        String[] selectionArgs = {planId+"",dataType};
        try {
     ...
     * @return
     */
    public boolean deleteUselessData(){
        long id = -1;
        String selection = COL_USER_ID + " = ?";
        String[] selectionArgs = {null};
        try {
            db.execSQL("delete from "+TABLE_NAME+" where "+COL_USER_ID+" is null");
        } catch (SQLException e) {
            e.printStackTrace();
        }
        closeDB();
    ...
    public boolean delete(String dataType){
        long id = -1;
        String selection = COL_DATA_TYPE + "=?";
     ...
     * @return
     */
    public List<DBBase> queryLocalAllPlan(){
        List<DBBase>  planList = new ArrayList<>();
        String[] columns = null;
        String selection = COL_DATA_TYPE + "=?";
        String[] selectionArgs = {Constants.DATA_TYPE_PLAN};
        Cursor cursor = db.query(TABLE_NAME, columns, selection, selectionArgs, null, null, null);
//        Cursor cursor = db.query(TABLE_NAME, null, null, null, null, null, null);
            ...
            try{
                while (cursor.moveToNext()){
                    ...
                    dbBase.setTaskId(cursor.getString(cursor.getColumnIndex(COL_TASK_ID)));
                    dbBase.setJson(cursor.getString(cursor.getColumnIndex(COL_DATA)));
                    planList.add(dbBase);
                }
            }catch (Exception e){
                e.printStackTrace();
